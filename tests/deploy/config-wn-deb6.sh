...
MPI_OPENMPI_PATH=/usr
MPI_OPENMPI_VERSION=1.4.2
EOF
diff_configs /tmp/env
...
MPI_MPICH2_PATH=/usr
MPI_MPICH2_VERSION=1.2.1.1
MPI_MPICH_PATH=/usr/lib/mpich
...
cat /tmp/mpi-start/tests/run_tests.sh | \
    ...
    sed 's/RUN_MPICH_TESTS=0/RUN_MPICH_TESTS=1/' |\
    sed 's/RUN_LAM_TESTS=0/RUN_LAM_TESTS=1/' |\
    sed 's/RUN_OPENMPI_TESTS=0/RUN_OPENMPI_TESTS=1/' > /tmp/mpi-start/tests/runner.sh
